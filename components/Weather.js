...
export default function Weather({navigation}) {
    ...
    const [everythingFullyLoaded, setEverythingFullyLoaded] = useState(false)
    ...
    useEffect( () => {
        const fetchAndSetData = async () => {
            ...
            const formatedWithDaysWeatherData = res.list.map( element => {
                return {...element, day: dayjs.unix(element.dt).locale('fr').format('dddd')}
            })
            let newForecastData = formatedWithDaysWeatherData.map(forecast => {
                return forecast.day
                ...
                return {
                    day,
                    data: formatedWithDaysWeatherData.filter((forecast) => forecast.day === day)
                }
            })
    ...
    useEffect( () => {
        ...
        if (formatedWeatherData != undefined) setEverythingFullyLoaded(true)
    ...
    useEffect( () => {
        console.log('raw:',weatherData)
        console.log('formated:',formatedWeatherData)
        console.log(everythingFullyLoaded)

    },[isPreview])


    // useEffect( () => {
    //     const formatDataWithDays = () => {
    //         const formatedWithDaysWeatherData = weatherData?.list.map( element => {
    //             return {...element, day: dayjs.unix(element.dt).locale('fr').format('dddd')}
    //         })
    //         let newForecastData = formatedWithDaysWeatherData?.map(forecast => {
    //             return forecast.day
    ...
    //     console.log('formated data',formatedWeatherData)
    // }, [weatherData])
    ...
    <ScrollView style={styleWeather.back}>
        <View style={styleWeather.main}>
            ...
            <>
                ...
                { mainWeather.icon ? 
                    <Image source={{ uri: `http://openweathermap.org/img/wn/${mainWeather.icon}@4x.png` }} style={styleWeather.image}  /> 
                : <></>}
                <View>
                ...
                <View>
                    <Text style={styleWeather.mainDt}>
                        {dayjs.unix(mainWeather.dt).locale('fr').format('DD MMMM YYYY  HH:mm')}
                    </Text>
                </View>
            </>
            : <></>}
            ...
            <>
                ...
                <View style={styleWeather.headerMoreInfo}>
                    { mainWeather.icon ? 
                        <Image source={{ uri: `http://openweathermap.org/img/wn/${mainWeather.icon}@4x.png` }} style={styleWeather.imageAlt}  /> 
                    : <></>}
                    <View>
                        ...
                        <View style={styleWeather.mainTempContainerAlt}>
                            ...
                            <Text style={styleWeather.mainTempAltMin} >{`/${Math.round(mainWeather.temp_min)}°`}</Text>
                        </View>
            ...
            <View style={styleWeather.details}>
                <View style={styleWeather.detailsChild}>
                    ...
                    {`${Math.round((mainWeather.wind*3.6))} km/h`}
                    </Text>
        ...
        <View style={styleWeather.container}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styleWeather.footer}>
                {weatherData?.list.map( (element,i) => {
                    return <View key={i} style={styleWeather.footerElement}>
                        <Text style={styleWeather.footerElementTemp}>
                            {`${Math.round(element.main.temp)}°`}
                        </Text>
                        <Image style={styleWeather.footerElementImage} source={{ uri: `http://openweathermap.org/img/wn/${element.weather[0].icon}@4x.png` }}/>
                        <Text style={styleWeather.footerElementHour}> 
                            {dayjs.unix(element.dt).locale('fr').format('HH:mm')}
                        </Text>
                    </View>
                })}
 ...
 ?
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styleWeather.footer}>
            {
            everythingFullyLoaded ? 
            formatedWeatherData.map( (day,i) => { 
                return <View key={i} style={styleWeather.container}>
                    <View style={styleWeather.footerHeader}>
                        <Text style={styleWeather.footerHeaderToday}>{day.day}</Text>
                        <Text style={styleWeather.footerHeaderMore} onPress={() => setIsPreview(true)}>Plus <FontAwesome style={styleWeather.footerHeaderIcon} name='chevron-right'/></Text>
                    </View>
